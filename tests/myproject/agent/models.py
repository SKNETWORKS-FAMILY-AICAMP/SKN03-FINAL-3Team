...
class hrdatabase_welfarebenefits(models.Model):
    employee_id = models.ForeignKey(
        hrdatabase_employee, on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    def __str__(self):
        ...
        return (
            f"Welfare for {self.employee_id.name}"
            if self.employee_id
            else "Welfare (no employee)"
        )
...
class hrdatabase_welfarepoints(models.Model):
    ...
    def __str__(self):
        return (
            f"Points for {self.employee_id.name}"
            if self.employee_id
            else "Points (no employee)"
        )
...
class hrdatabase_attendancemanagement(models.Model):
    ...
    def __str__(self):
        return (
            f"Attendance for {self.employee_id.name}"
            if self.employee_id
            else "Attendance (no employee)"
        )
...
class hrdatabase_attendancerecord(models.Model):
    ...
    employee_id = models.ForeignKey(
        hrdatabase_employee, on_delete=models.CASCADE
    )  # 직원 ID와 연결
    ...
    def __str__(self):
        return (
            f"{self.record_id} - {self.employee.name} ({self.date})"
            if self.employee
            else f"{self.record_id} (no employee)"
        )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["team_id", "employee_id"], name="unique_team_employee"
            )
        ]
